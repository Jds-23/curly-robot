...
const ProjectCarousel = ({ projects }) => {
    ...
    useEffect(() => {
        const timeout2 = setTimeout(() => {
            setState((prev) => (prev + 1) % projects.length);
        }, 4000);
    ...
    return (
        <Container>
            {projects.map(({ id, desc, projectName, imgSrc, techStack }) => (
...
display:grid;
gap:30px;
${props => props.isTablet ? "grid-template-rows:1fr 1fr;" : "grid-template-columns:minmax(336px,8fr) minmax(168px,4fr);"}
overflow:hidden;
height:auto;
&:nth-child(n+2){
position: absolute;
